...
> **当前进度**: 10% 开始
> **计划**: [plans/typed-munching-sphinx.md](../.claude/plans/typed-munching-sphinx.md)

### 功能概述

| 模式 | 快捷键 | 输出格式 |
|------|--------|----------|
| WikiLink | `Ctrl+Alt+K` | `[[文件名]]` 或 `[[文件名\|别名]]` |
| 块引用 | `Ctrl+Alt+Shift+K` | `((文件名#行号-行号))` |
| 快速链接 | `Ctrl+Alt+L` | `[[文件名\|选中文本]]` |

### 任务清单

| 顺序 | 任务 | 位置 | 工时 | 状态 |
|------|------|------|------|------|
| 1 | 添加 CodeMirrorEditorRef 接口 | `types.ts` | 30min | ⏳ 待开始 |
| 2 | 添加 ActionId 类型 | `types.ts` | 15min | ⏳ 待开始 |
| 3 | 添加 linkInsert 翻译 | `utils/translations.ts` | 30min | ⏳ 待开始 |
| 4 | 添加 forwardRef + useImperativeHandle | `CodeMirrorEditor.tsx` | 1h | ⏳ 待开始 |
| 5 | 添加 CodeMirror 6 keymap | `CodeMirrorEditor.tsx` | 1h | ⏳ 待开始 |
| 6 | 创建 LinkInsertModal 组件 | `components/LinkInsertModal.tsx` | 3h | ⏳ 待开始 |
| 7 | 集成链接插入功能 | `App.tsx` | 2h | ⏳ 待开始 |
| 8 | 测试验证 | 全项目 | 1h | ⏳ 待开始 |
